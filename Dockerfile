...
RUN apk add -U tzdata \
    ...
    && mv /tmp/config_file/cfssl/cfssl* /usr/local/bin/ \
    && chmod +x /usr/local/bin/cfssl* \
    && mv /tmp/config_file/ssl /nginx/ -f \
    && cd /nginx/ssl/ssl-conf \
    # 生成证书
    && cfssl gencert -initca ca-csr.json | cfssljson -bare ca && cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=www self-csr.json | cfssljson -bare self && cat self.pem ca.pem > ssl.crt && cp self-key.pem ssl.key && mv ssl.key /etc/nginx/ssl/ && mv ssl.crt /etc/nginx/ssl/ \
